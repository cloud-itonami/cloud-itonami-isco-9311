(ns mininglabor.actor
  "MiningLaborActor — the ISCO-08 9311 mining and quarrying labor
  coordination actor as a `langgraph.graph/state-graph` (ADR-2607121000
  / CLAUDE.md Actors section). One graph run = one labor-coordination
  ...
  The unconditional invariant: the MiningLaborAdvisor can never
  directly commit an operating record the MiningLaborGovernor
  refuses, and can never make this actor finalize a
  blast-authorization or mine-entry/area-clearance decision — every
  commit-record! call is gated behind `:decide`, and those two
  decision classes are a permanent hard block regardless of
  disposition path. This actor coordinates LABOR
  SCHEDULING/LOGISTICS ONLY — it never performs mining/quarrying work
  or authorizes site operations itself."
...
(defn build-graph
  "Build a compiled MiningLaborActor graph. `store` implements
  `mininglabor.store/Store`. `advisor` implements
  `mininglabor.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:laborer-id (:laborer-id request)
                                    :op (:op proposal)
